...
import { expenses, sales, farmers, stockBatches, financialSummary, categoryLabels, paymentLabels, products, type Expense, type Sale, type PaymentMethod, type Category, type Channel } from '../data/store'
import { fmtUGX, fmtDate, fmtPct, calcBatchMargin, totalExpenses, downloadCSV } from '../utils/format'
...
export default function BanelloPlatform() {
  ...
  function handleLogExpense() {
    ...
    const e: Expense = {
      ...
      paymentMethod: newExpense.method as PaymentMethod,
      paymentRef: newExpense.ref || undefined,
      attachedToBatch: newExpense.attach || undefined,
  ...
  function handleLogSale() {
    ...
    const s: Sale = {
      ...
      channel: 'kampala-premium' as Channel,
      customerName: newSale.customer,
      customerType: 'office',
      items: [{ productId: prod.id, productName: prod.name, grade: 'A', qty, unitPrice: price, lineTotal: qty * price }],
      subtotal: qty * price,
      deliveryFee: fee,
      totalAmount: qty * price + fee,
      paymentMethod: newSale.method as PaymentMethod,
      status: 'confirmed',
      linkedBatchId: newSale.batch || undefined,
